    ...
    <div className={styles.card}>
        ...
        {config["features"].map(
            (feat)=>(
                feat.long
                    ?
                <div key={feat.id}><p className={styles.feature}>{feat.label}</p><span>{item[feat.id]}</span></div>
                    :
                <p key={feat.id}><span className={styles.feature}>{feat.label}</span><span>{typeof item[feat.id]=== 'object' && item[feat.id] !== null? item[feat.id].name:item[feat.id]}</span></p>)
                )   }
        {"types" in config ? <div className={styles.types}>
            {config["types"].map((type_)=>
                (<div key={type_.id} className={`${styles.type} ${item[type_.id] ? styles.true : styles.false}`}>
