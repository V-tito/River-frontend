...
const DataTable = ({ data, kind }) => {
	...
	const aliases = {
		isOutput: { true: 'Исходящий', false: 'Входящий' },
		...
		<table className={styles.table}>
			...
			<tbody>
				{data.length > 0 ? (
					data.map(item => (
						<tr key={item.id}>
							{Object.entries(item).map(item1 =>
								item1[0] in config ? (
									item1[0] != 'description' ? (
										<td key={item1[0]} className={styles.td}>
											{item1[0] in aliases
												? aliases[item1[0]][item1[1]]
												: String(item1[1])}
										</td>
									) : null
								) : item1[0] == 'testBoard' ? (
							...
							<td className={styles.alterTd}>
								<AlterModal table={kind} obj={item}></AlterModal>
							</td>
						</tr>
					))
				) : (
