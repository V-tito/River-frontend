...
export async function postEntity(type, body) {
	...
	console.log('sent',JSON.stringify(body),' on ',`${process.env.API_URL}/api/river/v1/configurator/${type}`)
	if (!response.ok) {
		throw new Error(`Ошибка сети ${response.status}`);
...
export async function deleteEntity(type, name,group=null) {
	console.log('del on url', `${process.env.API_URL}/api/river/v1/configurator/${type}?name=${name}`)
	let api
